...
load('data.mat');
load('Agenda.mat');
timeindices=1:max(trajs(1).time)/5:max(trajs(1).time);
magindices=0.5:0.14:1.3;
[Agenda] =GenInitATree(timeindices, magindices, name);
[top] =GenTemporal(timeindices);
InitH=[];
fea=ExtractFeature(Agenda(1),1);
T=2;
N=5;
Loop=20;
Dfeature=length(fea);
lbound=-1*ones(Dfeature,1);
ubound=1*ones(Dfeature,1);

[Formula, Reward,Theta] =ActiveLearning(InitH,Agenda,top,lbound,ubound,T,Loop,trajs,label,name);
figure
plot(Reward)
[re,index]=max(Reward);
